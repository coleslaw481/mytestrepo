...
This is an advanced developer section that describes creation of a new release for mytestrepo and deploying
...
Be sure to update the version in ``mytestrepo/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' mytestrepo/__init__.py
...
From mytestrepo source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- mytestrepo-0.1.0-py2.py3-none-any.whl
    `-- mytestrepo-0.1.0.tar.gz
...
#. From https://github.com/coleslaw481/mytestrepo click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/mytestrepo and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi mytestrepo
...
.. _Travis: https://travis-ci.org/coleslaw481/mytestrepo
.. _main: https://github.com/coleslaw481/mytestrepo/tree/main
